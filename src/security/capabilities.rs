...
impl CapabilityManager {
    ...
    /// Phase 1 variant that preserves a small post-identity capability set.
    ///
    /// The identity-switch capabilities are kept temporarily because setuid/setgid
    /// happens between phase 1 and phase 2. Call [`finalize_drop_except`] after
    /// switching identity to remove those temporary capabilities.
    pub fn drop_bounding_set_except(&mut self, keep_after_identity: &[Capability]) -> Result<()> {
        if self.phase != CapPhase::Initial {
            debug!("Bounding set already dropped, skipping");
            return Ok(());
        }

        let mut keep_during_identity = to_caps_hash_set(keep_after_identity);
        keep_during_identity.insert(Capability::CAP_SETUID);
        keep_during_identity.insert(Capability::CAP_SETGID);
        keep_during_identity.insert(Capability::CAP_SETPCAP);

        info!(
            "Phase 1: dropping bounding set except {:?}",
            keep_during_identity
        );

        for cap in caps::all() {
            if !keep_during_identity.contains(&cap) {
                if let Err(e) = caps::drop(None, CapSet::Bounding, cap) {
                    debug!(
                        "Failed to drop bounding cap {:?}: {} (may not be present)",
                        cap, e
        ...
        let leaked: Vec<String> = bounding
            .iter()
            .filter(|cap| !keep_during_identity.contains(cap))
            .map(|cap| format!("{:?}", cap))
            .collect();
        if !leaked.is_empty() {
            return Err(NucleusError::CapabilityError(format!(
                "Bounding set still contains unexpected capabilities after drop: [{}]",
                leaked.join(", ")
            )));
        }
        ...
        self.phase = CapPhase::BoundingDropped;
        info!("Phase 1 complete: bounding/ambient/inheritable reduced");
    ...
    /// Phase 2 variant that keeps a constrained permitted/effective set.
    pub fn finalize_drop_except(&mut self, keep: &[Capability]) -> Result<()> {
        if self.phase == CapPhase::Dropped {
            debug!("Capabilities already reduced, skipping");
            return Ok(());
        }

        if self.phase == CapPhase::Initial {
            self.drop_bounding_set_except(keep)?;
        }

        let keep_set = to_caps_hash_set(keep);
        info!("Phase 2: clearing capabilities except {:?}", keep_set);
        ...
        let permitted = caps::read(None, CapSet::Permitted).map_err(|e| {
            NucleusError::CapabilityError(format!("Failed to read permitted caps: {}", e))
        })?;
        ...
        let leaked: Vec<String> = effective
            .iter()
            .filter(|cap| !keep_set.contains(cap))
            .map(|cap| format!("{:?}", cap))
            .collect();
        if !leaked.is_empty() {
            return Err(NucleusError::CapabilityError(format!(
                "Effective set still contains unexpected capabilities after drop: [{}]",
                leaked.join(", ")
            )));
        }

        self.phase = CapPhase::Dropped;
        info!("Successfully reduced capabilities");
